...
function hrefBuilder(
    basePath: string, page: number, extraParams?: { query?: string; tag?: string }
) {
    ...
    return (
        <nav className="flex items-center justify-between pt-6 text-sm">
            <Link href={hrefBuilder(basePath, Math.max(1, currentPage - 1), extraParams)} scroll={false}
                className={`w-24 text-center rounded-lg border px-3 py-2 ${currentPage === 1
                ...
                {Array.from({ length: totalPages }).map((_, idx) => {
                    ...
                    return (
                        <Link
                            ...
                            className={`h-9 w-9 rounded-lg text-center leading-9 ${isActive
            ...
            <Link href={hrefBuilder(basePath, Math.max(totalPages, currentPage + 1), extraParams)} scroll={false}
