  ...
  return (
    <div className="space-y-12">
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((item) => (
          <div key={item.name} className="p-6 rounded-2xl border bg-white hover:bg-gray-50 transition-colors">
            <div className="flex flex-col space-y-2">
                <item.icon className="h-5 w-5 text-gray-400" />
                <h3 className="text-sm font-medium text-gray-500">{item.name}</h3>
                <div className="flex items-center">
                    {item.value === "..." && <Loader2 className="h-4 w-4 animate-spin mr-2 text-gray-300" />}
      ...
      <section className="space-y-4">
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 p-12 flex flex-col items-center justify-center text-gray-400">
