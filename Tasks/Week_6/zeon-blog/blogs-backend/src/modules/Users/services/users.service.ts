...
export class UsersService {
    ...
    async updateCurrentUserProfile(currentUser: CurrentUser, dto: UpdateUserProfileDto) {
        const user = await this.userRepository.findOne({
        ...
        if (dto.role !== undefined) {
            if (currentUser.role !== UserRole.ADMIN) throw new ForbiddenException("Only Admin can update user role");
            user.role = dto.role;
        }
    ...
    async getAllUsers(queryDto: QueryUsersDto, currentUser: CurrentUser) {
        if (currentUser.role !== UserRole.ADMIN) throw new ForbiddenException("Only Admin can get all users");
    ...
    async deleteUser(id: string, currentUser: CurrentUser) {
        ...
        if (!user) throw new NotFoundException("User Not Found");
        if (currentUser.role !== UserRole.ADMIN) throw new ForbiddenException("Only Admin can delete user");
