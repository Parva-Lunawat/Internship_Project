...
export class BlogsService {
    ...
    private async ensureUniquePageTitle(pageTitle: string, checkingBlogId?: string): Promise<void> {
        const normalized = this.normalizedPageTitle(pageTitle);
        if (this.isReservedPageTitle(normalized)) throw new BadRequestException('This is a reserved PageTitle.')
        const existing = await this.blogsRepository.findOne({
            where: {
                pageTitle: normalized,
            },
        });
        if (existing && existing.id !== checkingBlogId) throw new BadRequestException('PageTitle Already Exists.')
    }
    private async resolveTags(tagNames?: string[]): Promise<Tag[]> {
        ...
        const savedNewTags = tagsToCreate.length > 0 ? await this.tagsRepository.save(tagsToCreate) : [];
    ...
    private applyPublishedAtOnUpdate(existingBlog: Blog, newStatus?: BlogStatus): Date | null {
        const previousState = existingBlog.status;
        const newState = newStatus ?? previousState;

        if (previousState === BlogStatus.PUBLISHED && newState !== BlogStatus.DRAFT) {
            return null;
        }
        if (previousState === BlogStatus.DRAFT && newState === BlogStatus.PUBLISHED) {
            return new Date();
        }
    ...
    async getMyBlogsById(blogId: string, currentUser: CurrentUser) {
        ...
        if (!this.canManageBlog(blog, currentUser)) throw new ForbiddenException("You are not authorized to perform this action.");
        return this.mapBlogResponse(blog);
    }
    ...
    async updateBlog(blogId: string, dto: UpdateBlogDto, currentUser: CurrentUser) {
        ...
        if (!this.canManageBlog(updateBlog, currentUser)) throw new ForbiddenException("You are not authorized to perform this action.");
