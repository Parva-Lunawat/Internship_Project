...
export const showInitNotes = (searchQuery = "") => async (dispatch) => {
    try {
        const notes = await api.listNotes(searchQuery);
        // notes is already the array because of your "unwrap" in request()
...
export const noteCreation = (draft) => async (dispatch) => {
    try {
        const newNote = await api.createNote(draft);
...
export const updateNoteFunc = (id, updates) => async (dispatch) => {
    try {
...
export const toggleStarFunc = (id) => async (dispatch) => {
    try {
        const toggledNote = await api.toggleStar(id);
        dispatch(notesToggleStar(id));
    } catch (err) {
        console.log("Starring failed:", err);
    }
}

export const deleteNotesFunc = (id) => async (dispatch) => {
    try {
